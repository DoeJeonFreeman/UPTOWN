...
//		thisMovie('SFS_meteogram.swf').sendMsgToFlexApp(msg); //do not use this stuff
	...
	function getCurrentStatusForSwf() {
		var swfDataType = $("input:radio[name=rdo_subTabPoint]:checked").attr("ref");
		var swfXmlType = $("input:radio[name=rdo_subTabModePoint]:checked").attr("ref");
		var swfXmlType1 = $("input:radio[name=rdo_subTabModePoint]:checked").val();
		var swfDateTime = $("#floatingMenuPoint option:selected").attr("ref");
		var swfStationNo = $("#stndate option:selected").val();
		
		
		//doejeon meFine
		if ((swfXmlType1=="PYEONGCHANG" || swfXmlType1=="etc")&&(swfDataType=="POP")) {
               ////console.log(swfXmlType1 + ' \n ' + swfDataType + '\n' + swfXmlType)
			   swfXmlType1 = "SSPS";
               swfXmlType = "DFS_SHRT_STN_SSPS";
			   swfDataType = "VIS";
		}//doejeon
		
		
		if ((swfXmlType1=="GDPS")&&(swfDataType=="T3H")) {
               swfXmlType1 = "PMOS2";
               swfXmlType = "DFS_MEDM_STN_GDPS_PMOS";
			swfDataType = "MMX";
		}

 		if ((swfXmlType1=="GDPS")&&(swfDataType=="SN3")) {	//20130603 전수재 
               swfXmlType1 = "GDPS";
               swfXmlType = "DFS_MEDM_STN_GDPS_NPPM";
			swfDataType = "S12";
		} else if ((swfXmlType1=="GDPS")&&(swfDataType=="RN3")) {
               swfXmlType1 = "GDPS";
               swfXmlType = "DFS_MEDM_STN_GDPS_NPPM";
			swfDataType = "R12";
		}

       if ((swfXmlType1=="w_ECMW")&&(swfDataType=="T3H")) {
              swfDataType = "MMX";
              }

       if ((swfXmlType1=="w_ECMW")&&(swfDataType=="SN3")) {	//20130603 전수재
              swfDataType = "S12";
       } else if ((swfXmlType1=="w_ECMW")&&(swfDataType=="RN3")) {
              swfDataType = "R12";
       }
	   
	   if ((swfXmlType1=="EPSG")&&(swfDataType=="T3H")) {
              swfDataType = "MMX";
              }

       if ((swfXmlType1=="EPSG")&&(swfDataType=="SN3")) {	//20130603 전수재
              swfDataType = "S12";
       } else if ((swfXmlType1=="EPSG")&&(swfDataType=="RN3")) {
              swfDataType = "R12";
       }
		
		//doejeon meBest
		if(swfXmlType.indexOf("BEST") != -1 && swfXmlType.indexOf("MEDM") != -1){
		...
		if ((swfDataType=="MUL")) {
            if (( swfXmlType1 =="GDPS" ) || ( swfXmlType1 =="PMOS2" ) ){
            	swfXmlType = "DFS_MEDM_STN_GDPS_PMOS";
                swfParam = swfXmlType + " " + "MMX" + " " + swfDateTime + " " + swfStationNo + " isMulti";
            }else {
            	var temperature = (swfXmlType.split("_")[1]=="SHRT")? "T3H":"MMX";
				swfParam = swfXmlType + " " + temperature + " " + swfDateTime + " " + swfStationNo + " isMulti";
              // alert('getCurren~()' + swfParam);
			}
			
			if(swfXmlType1=="PYEONGCHANG"){swfParam+=" isPYEONGCHANG2018"}
		}              

//		////console.log("getCurrentStatusForSwf() swfParam == " + swfParam);
		return swfParam;
	}
	...
	function checkWhetherMergedModelCheckboxIsCheckedOrNot(isChecked){
		////console.log('checkWhetherMergedModelCheckboxIsCheckedOrNot\(isChecked\)\nisChecked == ' + isChecked);
...
//			////console.log(model_seq);
	...
	function getInitInfo(userConfiguredGroupCode, baseModel, isMEDM, isPyeongchang) {
		...
		//	}
			...
			// //console.log('checkDisabledSubTabs(currentSubTab)::  ' + currentSubTab)
			// 서브메뉴를 선별하여 보여준다
			checkDisabledSubTabs(currentSubTab);
		}
...
//		////console.log('isFileThere():: param fileNameStr == ' + fileNameStr + " [isFileExist.php]" + fileName);      
		var result = $.ajax({
			url: "isFileExist.php",
			type: "GET",
